...
public class AgentBusinessLogic {
    ...
    public DetailAgent [] retrieveAgents(String param) throws IOException, JSONException {
        ...
        JSONObject agentIndex = agent.getJSONObject(0);
        detailAgent = new DetailAgent[numOfAgent+1];
        for (Integer i=0; i <= numOfAgent; i++){
            JSONObject agentDetail = new JSONObject(agentIndex.getString(i.toString()));
